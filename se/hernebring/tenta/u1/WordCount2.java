...
public class WordCount2 {
    public static void main(String[] args) {
        if (args.length != 2 || !args[0].endsWith(".txt") || !args[1].endsWith(".txt") || args[0].equals(args[1])) {
            ...
            Map<String, Integer> countWords = count(words);
            int uniqueWords = extractUnique(countWords);
            ...
            try (BufferedWriter writer = Files.newBufferedWriter(Path.of(args[1]), StandardOpenOption.CREATE_NEW)) {
                ...
                writer.newLine();
                writer.write("Total unique words: " + uniqueWords);
            } catch (IOException ex) {
    ...
    private static int extractUnique(Map<String, Integer> countWords) {
        int uniqueWords = 0;
        for(Entry<String,Integer> countWord: countWords.entrySet()){
            if(countWord.getValue() == 1){
                uniqueWords++;
            }
        }
        return uniqueWords;
